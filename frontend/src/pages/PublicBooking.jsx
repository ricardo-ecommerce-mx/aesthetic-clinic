    ...
    <div style={{ minHeight: '100vh', background: 'var(--teal-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ background: 'white', borderRadius: 'var(--radius-2xl)', padding: 48, maxWidth: 440, width: '100%', textAlign: 'center', boxShadow: 'var(--shadow-xl)' }}>
        ...
        <p style={{ color: 'var(--gray-500)', marginBottom: 24, lineHeight: 1.6 }}>
          Hola <strong>{form.name}</strong>, tu cita para el <strong>{MONTHS[month]} {selectedDay}</strong> a las <strong>{selectedSlot}</strong> ha sido registrada.<br /><br />
          Recibirás una confirmación por WhatsApp al número {form.phone}.
        </p>
        <div style={{ background: 'var(--teal-50)', borderRadius: 'var(--radius-lg)', padding: '12px 16px', marginBottom: 24, fontSize: 13, color: 'var(--teal-700)', fontWeight: 600 }}>
          {form.service}
        </div>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto 32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--gray-400)' }}>Agenda tu cita en línea</div>
          </div>
        </div>
        <h1 style={{ fontSize: 28, fontWeight: 800, color: 'var(--gray-900)', marginBottom: 6 }}>Reserva tu cita</h1>
        <p style={{ color: 'var(--gray-500)', fontSize: 15 }}>Elige el día, horario y completa tus datos. ¡Es rápido y sencillo! 💆‍♀️</p>
      </div>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20 }}>
            ...
            {Array(daysInMonth).fill(null).map((_, i) => {
              ...
              return (
                <button
                  ...
                  style={{
                    padding: '8px 4px', textAlign: 'center', border: isSel ? '2px solid var(--teal-400)' : 'none',
        ...
        <div style={{ background: 'white', borderRadius: 'var(--radius-xl)', padding: 24, boxShadow: 'var(--shadow)', border: '1px solid var(--gray-100)' }}>
          {!selectedDay ? (
            ...
            <>
              <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--gray-900)', marginBottom: 16 }}>
                ...
                {ALL_SLOTS.map(slot => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        padding: '8px', textAlign: 'center', fontFamily: 'inherit',
              ...
              {selectedSlot && (
                <form onSubmit={handleConfirm} className="fade-in">
                  ...
                  <div className="form-group">
                    ...
                    <input className="form-input" placeholder="Tu nombre" required value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Correo electrónico</label>
                    <input className="form-input" type="email" placeholder="correo@ejemplo.com" required value={form.email} onChange={e => setForm({...form, email: e.target.value})} />
                  </div>
                  <div className="form-group">
                    <label className="form-label">WhatsApp</label>
                    <input className="form-input" type="tel" placeholder="+52 222 123 4567" required value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} />
                  </div>
                  <div className="form-group">
                  ...
                  <button type="submit" className="btn btn-primary btn-block">✓ Confirmar cita</button>
                </form>
              )}
            </>
          )}
